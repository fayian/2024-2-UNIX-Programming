...
syscall_hook_fn_t hook_fn = enterSyscall;
...
static int do_rewrite(void *data, const char *fmt, ...) {
	...
	// For bypassing "redzone". It gives up and asssert(0) when it can't. But there is a chance that it doesn't matter.
...
static void rewriteCode(void) {
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL) {
        if(strstr(buf, "[stack]\n") != NULL ||            
           strstr(buf, "[vdso]\n") != NULL) {
            continue; //we do not touch stack and vsdo memory
        }
            ...
            case 1:
                {
                    ...
                    if (mem_prot & PROT_EXEC) {
                        ...
                        uintptr_t from, to;
                        from = strtoul(&addr[0], NULL, 16);
						printf("%p\n", (void*)from);
                        if (from == 0 || from == 0xffffffffff600000) {
							// skip trampoline code and vsyscall
                            break;
                        }
...
static void loadHookLib(void) {
	...
	assert(hook_init);
	hook_init(enterSyscall, &hook_fn);
}
